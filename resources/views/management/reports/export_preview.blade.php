...
@section('title', 'Assessment Summary & Export')
    ...
    <div class="bg-white p-4 rounded-2xl shadow-sm border border-gray-100 flex-1 w-full">
        <form method="GET" class="flex flex-col md:flex-row items-end gap-4">
            <div class="flex-1 w-full">
                <label class="block text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1.5 ml-1">Select Assessment Period</label>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden group">
        ...
        <div class="relative z-10">
            <p class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Total Employees</p>
            <h3 class="text-3xl font-extrabold text-gray-800">{{ $summary['total_employees'] }}</h3>
            <p class="text-xs text-blue-600 mt-2 font-medium">All Departments</p>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden group">
        ...
        <div class="relative z-10">
            <p class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Completed Assessments</p>
            <h3 class="text-3xl font-extrabold text-gray-800">{{ $summary['completed_assessments'] }}</h3>
            <p class="text-xs text-emerald-600 mt-2 font-medium">{{ round(($summary['completed_assessments'] / max(1, $summary['total_employees'])) * 100, 1) }}% Participation</p>
        </div>
    </div>
    ...
    <div class="bg-white p-6 rounded-2xl shadow-sm border border-gray-100 relative overflow-hidden group">
        ...
        <div class="relative z-10">
            <p class="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">Average Final Score</p>
            <h3 class="text-3xl font-extrabold text-gray-800">{{ number_format($summary['avg_score'], 2) }}</h3>
            <p class="text-xs text-purple-600 mt-2 font-medium">National Average</p>
        </div>
    </div>
</div>
...
<div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-50 flex justify-between items-center">
        <h4 class="font-bold text-gray-800">Summary by Department</h4>
    </div>
    <div class="overflow-x-auto">
        <table class="min-w-full divide-y divide-gray-100">
            <thead class="bg-gray-50/50">
                <tr>
                    <th class="px-6 py-4 text-left text-xs font-bold text-gray-400 uppercase tracking-wider">Department Name</th>
                    <th class="px-6 py-4 text-center text-xs font-bold text-gray-400 uppercase tracking-wider">Assessed Employees</th>
...
<div class="bg-white p-12 rounded-3xl shadow-sm border border-gray-100 text-center">
    ...
    <h3 class="text-xl font-bold text-gray-800 mb-2">No Assessment Period Found</h3>
    <p class="text-gray-500 max-w-xs mx-auto">Please create an assessment period first to see the report summary.</p>
</div>
@endif
@endsection
